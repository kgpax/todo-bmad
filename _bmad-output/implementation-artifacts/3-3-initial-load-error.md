...
- E2E kill approach: `pkill -9 -f 'tsx.*conditions=development'` kills only the backend tsx process. Does NOT kill concurrently, the Next.js frontend, or Playwright workers. `afterAll` restarts the backend standalone (not through concurrently) and polls until `localhost:3001/api/todos` responds OK.
- 214 unit tests pass, 100% coverage all packages. 24 E2E tests pass. Lighthouse: Desktop 95, Mobile 100, all categories 100. Linter and build clean.
